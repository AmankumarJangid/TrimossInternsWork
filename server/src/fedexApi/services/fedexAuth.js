...
class FedExAuth {
  ...
  async getAccessTokenForTracking () {
    if (this.accessTokenTracking && this.tokenExpiryTracking && new Date() < this.tokenExpiryTracking) {
      return this.accessTokenTracking
    }
    ...
    try {
      ...
      this.tokenExpiryTracking = new Date(
        Date.now() + response.data.expires_in * 1000 - 60000
      ) // 1 minute buffer
