...
const addressSchema = joi.object({
  streetLines: joi.array().items(joi.string().required()),
  ...
  const schema = joi.object({
    ...
    packages: joi
      ...
      .items(
        joi.object({
          weight: joi.object({
            units: joi.string().valid('KG', 'LB').required(),
    ...
    shipper: joi
      .object({
        ...
        phone: joi.string().required(),
            ...
            // .required()
            // .messages({
            //   'string.pattern.base': 'Phone number must be 10 to 15 digits, optionally starting with a + and a non-zero country code.',
