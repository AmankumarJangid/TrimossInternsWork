...
class FedExService {
      ...
      const payload = {
        ...
        requestedShipment: {
          shipper: {
            ...
            address: {
              streetLines: [shipmentData.shipper.address.street],
          ...
          recipients :{
            ...
            address: {
              streetLines: [shipmentData.recipient.address.street],
          ...
          requestedPackageLineItems: shipmentData.packages.map((pkg, index) => ({
  ...
  formatShipmentResponse(data) {
    ...
    return {
      trackingNumber: shipment.masterTrackingId.trackingNumber,
      labelUrl: shipment.pieceResponses[0].packageDocuments[0].url,
      totalCharges: shipment.shipmentAdvisoryDetails.regulatoryAdvisory.prohibitions,
      shipmentId: shipment.masterTrackingId.formId
    };
  }

  formatTrackingResponse(data) {
    const trackingInfo = data.output.completeTrackingResults[0].trackingResults[0];
    return {
      ...
      events: trackingInfo.scanEvents?.map(event => ({
